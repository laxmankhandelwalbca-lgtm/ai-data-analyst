...
if "conversation_history" not in st.session_state:
    st.session_state.conversation_history = []   # stores {question, sql, result_summary}
...
Choose the best chart type and columns. Return ONLY valid JSON, no explanation:
{{
...
"""
        try:
            ...
            fixed_sql = clean_sql(fix_resp.content[0].text)
...
"""
    ...
    return clean_sql(resp.content[0].text)
...
def explain_sql(sql: str) -> str:
    resp = client.messages.create(
        model=MODEL,
        max_tokens=150,
        messages=[{"role": "user", "content": f"Explain this SQL in simple plain English in 2-3 lines:\n\n{sql}"}]
    )
    return resp.content[0].text
...
st.caption("Upload CSVs → Ask questions in plain English → Get SQL, Charts & Insights")
...
with st.sidebar:
    ...
    if st.session_state.conversation_history:
        ...
        for i, h in enumerate(reversed(st.session_state.conversation_history[-5:])):
...
if uploaded_files:
    ...
    #  Multi-chart dashboard from one prompt
    # ══════════════════════════════════════════════════════════════════════════
    else:
        st.subheader("📊 Dashboard Builder")
        st.caption("Describe your dashboard and Claude will build multiple charts at once")
        ...
        if st.button("🚀 Generate Dashboard", type="primary"):
            with st.spinner("🤖 Planning your dashboard..."):

                # Ask Claude to split into individual chart questions
                split_prompt = f"""
...
"""
                try:
                    split_resp = client.messages.create(
                        model=MODEL,
                        max_tokens=500,
                        messages=[{"role": "user", "content": split_prompt}]
                    )
                    raw = split_resp.content[0].text.strip()
                    raw = raw.replace("```json", "").replace("```", "").strip()
...
"""
                    ...
                    st.write(insight_resp.content[0].text)
                except:
    ...
    with col2:
        ...
        st.caption("Describe a full dashboard in one sentence. Claude builds multiple charts automatically in a grid layout.")
    with col3:
        st.markdown("### 🧠 AI-Powered")
        st.caption("Claude chooses the best chart type, auto-fixes broken queries, and generates business insights from your data.")
